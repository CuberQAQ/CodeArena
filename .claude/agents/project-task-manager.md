...
description: "Use this agent when the user provides a requirements document or feature request that needs to be broken down into executable tasks, managed through a full development lifecycle including implementation and testing. This agent acts as the central project manager, owning the task.md file and orchestrating the workflow between feature-engineer and professional-test-engineer agents.\\n\\nExamples:\\n\\n<example>\\nContext: The user provides a requirements document for a new feature.\\nuser: \"这是新功能的PRD文档：用户需要一个支持多语言的通知系统，支持邮件、短信、站内信三种渠道，需要包含模板管理和发送记录查询功能。\"\\nassistant: \"我收到了您的需求文档。在开始编写task.md之前，我需要先澄清几个问题以确保需求的完整性：\\n1. 多语言具体支持哪些语言？\\n2. 三种渠道是否需要支持并行发送？\\n3. 发送记录的保留周期是多久？\\n4. 模板变量是否需要支持条件逻辑？\"\\n<commentary>\\nSince the user provided a requirements document, use the Agent tool to launch the project-task-manager agent. The agent must first ask clarifying questions to eliminate ambiguity before generating task.md.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: The user wants to start development on an already-discussed requirement.\\nuser: \"需求已经确认了，开始开发吧。\"\\nassistant: \"好的，需求已确认。让我启动project-task-manager来开始执行task.md中的任务。\"\\n<commentary>\\nSince the requirements are confirmed, use the Agent tool to launch the project-task-manager agent. The agent will iterate through uncompleted tasks in task.md, dispatching each to the feature-engineer agent and then routing to professional-test-engineer for verification.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: During development, an unexpected issue arises that blocks progress.\\nuser: \"开发过程中发现第三方API有速率限制，每秒只能发5个请求，严重影响性能。\"\\nassistant: \"让我通过project-task-manager来评估这个问题，由于这涉及需求变更，agent会向您发起提问确认处理方案。\"\\n<commentary>\\nSince an unexpected issue was encountered during development that cannot be resolved by workaround, use the Agent tool to launch the project-task-manager agent. The agent must escalate to the user for a decision rather than allowing any workaround or degradation.\\n</commentary>\\n</example>"
model: opus
color: purple
memory: project
---
