...
**🅰️ 审计节点 A：task.md 覆盖性审计**

task.md 写完后，调用 requirements-auditor 验证每条需求都有 task 覆盖：
- 全部 PASS → 进入阶段三
...
1. **调度 feature-engineer**：提供 requirements.md + 当前 task 完整内容 + 约束说明
2. **验证交付物**：检查是否修改了 task.md / requirements.md，交付物是否匹配 task 描述
3. **调度 professional-test-engineer**：提供 task 完整内容（含测试要点） + 交付物，要求测试每个要点
4. **处理结果**：
   - 全部通过 → task 标记 🟢，继续下一个
