...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "opponent_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False
        ),
    ...
    op.create_index(
        "ix_challenge_sessions_challenger_created", "challenge_sessions", ["challenger_id", sa.text("created_at DESC")]
    )
    op.create_index(
        "ix_challenge_sessions_opponent_created", "challenge_sessions", ["opponent_id", sa.text("created_at DESC")]
    )
