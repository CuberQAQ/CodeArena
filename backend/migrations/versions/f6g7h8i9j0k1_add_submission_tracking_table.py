...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "session_id", postgresql.UUID(as_uuid=True), nullable=False, comment="FK to the corresponding session table"
        ),
        ...
        sa.Column(
            "matched_at", sa.DateTime(timezone=True), nullable=True, comment="When the CF submission was matched"
        ),
    ...
    op.create_index(
        "ix_submission_tracking_user_session", "submission_tracking", ["user_id", "session_type", "session_id"]
    )
